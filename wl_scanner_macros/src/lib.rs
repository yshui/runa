...
pub fn generate_protocol(tokens: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let path = std::path::Path::new(&std::env::var("CARGO_WORKSPACE_DIR").unwrap()).join(&path.value()).to_owned();
    let tokens: proc_macro2::TokenStream = tokens.into();
    let mut file = std::fs::File::open(path).map_err(|e| syn::Error::new_spanned(&tokens, e)).unwrap_or_abort();
